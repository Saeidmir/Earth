...
namespace Earth.Caching.Distributed.InMemory.Services;

public class InMemoryCacheAdapter: ICahceAdapter.ICacheAdapter
{
    ...
    public void Add<TInput>(string key, TInput obj, DateTime? absoluteExpiration, TimeSpan? slidingExpiration)
    {
        _logger.LogTrace("InMemoryCache Adapter Cache {obj} with key: {key} "+
                         ...
                         ", with slidingExpiration : {slidingExpiration}", typeof(TInput)
                         );
